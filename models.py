...
class Seq2Seq(nn.Module):
    ...
    def forward(self, cw_idxs, qw_idxs):
        ...
        _, log_probs = self.decode(dec_init_state, qw_idxs)
        return log_probs
    ...
    def decode(self, dec_init_state: Tuple[torch.Tensor, torch.Tensor], qw_idxs: torch.Tensor) -> torch.Tensor:
        ...
        for q_t in torch.split(q_emb, split_size_or_sections=1, dim=1):         #(batch_size, 1, hidden_size)
            ...
            #dec_state, o_t = self.step(
            #    q_t,
            #    dec_state,
            #    enc_hiddens)
            dec_state = self.decoder(q_t, dec_state)
            h_t, _ = dec_state
            o_t = h_t 
    ...
    def step(self, q_t: torch.Tensor,
            dec_state: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[Tuple, torch.Tensor, torch.Tensor]:
        ...
        #a_t = toarch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens).squeeze(dim=1)
        #U_t = torch.cat((dec_hidden, a_t), dim=1)
        #V_t = self.projection(U_t)
        #O_t = self.dropout(torch.tanh(dec_hidden))
        output = dec_hidden
